...
const arrayB = [1, 1, 2, 2, 3, 4, 4, 5, 5, 6];
const arrayA = arrayB.filter((element, index) => {
  return arrayB.indexOf(element) == index;
});
...
const arrayToObject = (array) => {
  return array.map((value, index) => ({ index: index, value: value }));
};
...
const nameToAge = (person2, targetAge) => {
  return person2.find((person) => person.age === targetAge);
};
